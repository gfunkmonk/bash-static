...
mycurl() {
    ...
    if [[ ! ${NO_SIGS:-} ]]; then
        ...
        gpg --trust-model always --verify "${filename}.${sig_ext}" "${filename}" 2>/dev/null || {
            echo -e "${RED}ERROR: GPG verification failed for ${filename}${NC}" >&2
            return 1
        }
    fi
}
...
main() {
  ...
  echo -e "${BWHITE}Building for: ${VIOLET}OS=${target}, ${CRIMSON}ARCH=${arch}${NC}"
  ...
  for patch in ../custom/bash"${bash_version/\./}"*.patch; do
        if [[ -f "$patch" ]]; then
            echo -e "${BWHITE}Applying ${patch##*/}${NC}"
            pushd bash-"${bash_version}"
            patch -sp1 <../"${patch}" || {
  ...
  if [[ "$target" != "macos" ]]; then
    ...
    make -j"$(sysctl -n hw.physicalcpu)" -s
  fi
